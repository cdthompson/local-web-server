...
function localWebServer (options) {
  ...
  /* serve static files, using koa-stream for Range support */
  if (options.static.root) {
    const stream = require('koa-stream');
    app.use(function *() {
      var filename = this.path.substr(1);
      console.log("Serving " + filename + " from " + options.static.root);
      yield stream.file(this, filename, {root: options.static.root, allowDownload: true});
    });
  }
